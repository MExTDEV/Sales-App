  ...
  return (
    <section className="grid gap-2">
      <div className="flex items-center justify-between px-1 pt-0.5">
        ...
        <span className="rounded-full bg-slate-200 px-2.5 py-0.5 text-xs font-black text-slate-700">{items.length}</span>
      </div>
...
function AppointmentCard({
  ...
  return (
    <article
      className={`grid gap-2.5 rounded-lg border bg-white px-3 py-2.5 shadow-[0_8px_22px_rgba(15,23,42,0.05)] transition hover:-translate-y-0.5 hover:shadow-[0_14px_34px_rgba(15,23,42,0.09)] lg:grid-cols-[4.3rem_1fr_auto] lg:items-center ${statusAccent(item.status)} ${
        selectedId === item.id ? "border-[#003B83] ring-2 ring-blue-100" : "border-slate-200"
      }`}
    >
      <button className="rounded-md bg-slate-50 px-2 py-2 text-left transition hover:bg-blue-50" onClick={openDetail}>
        <span className="block text-lg font-black leading-none tracking-tight text-slate-950">{item.time}</span>
        <span className="mt-1.5 inline-flex rounded-md border border-slate-300 bg-white px-1.5 py-0.5 text-[0.58rem] font-black uppercase text-slate-700">
      ...
      <button className="min-w-0 text-left" onClick={openDetail}>
        <span className="flex flex-wrap items-center gap-2">
          <span className={`rounded-full px-2 py-0.5 text-[0.62rem] font-black uppercase tracking-wide ${item.customer ? "bg-cyan-100 text-cyan-800" : "bg-amber-100 text-amber-800"}`}>
            {item.customer ? t("appointment.customer") : t("appointment.prospect")}
          </span>
          <span className="rounded-full bg-slate-100 px-2 py-0.5 text-[0.62rem] font-black text-slate-700">{account?.number}</span>
          <StatusBadge status={item.status} label={t(`status.${item.status}`)} />
          {item.label && <span className="rounded-full bg-amber-300 px-2 py-0.5 text-[0.62rem] font-black text-amber-950">{item.label}</span>}
        </span>
        <span className="mt-1.5 block text-sm font-black tracking-tight text-slate-950">{account?.name}</span>
        <span className="mt-1.5 grid gap-1.5 text-xs font-medium text-slate-600 md:grid-cols-[1fr_minmax(13rem,0.7fr)]">
      ...
      <div className="grid w-full gap-2 sm:w-[15rem] lg:justify-self-end">
...
function AgendaAction({
  ...
  return (
    <button
      className={`inline-flex min-h-9 items-center justify-center gap-1.5 rounded-md border px-3 text-xs font-bold shadow-sm transition ${fullWidth ? "w-full" : "w-full"} ${
        primary
...
function RevenueBlock({ appointment, t }: { appointment: Appointment; t: TranslationFn }) {
  return (
    <div className="rounded-md border border-slate-200 bg-slate-50 p-2">
      <p className="mb-1 text-[0.6rem] font-black uppercase tracking-wide text-slate-500">{t("agenda.revenue.title")}</p>
...
function EmptyState({ message }: { message: string }) {
  return (
    <div className="grid min-h-24 place-items-center rounded-lg border border-dashed border-slate-300 bg-white p-4 text-center shadow-[0_8px_22px_rgba(15,23,42,0.04)]">
      <div>
